...
public:
	...
	TriSparseMatrix<ElemType> operator +(const TriSparseMatrix<ElemType>& copy);//加法运算符重载
};
...
// 操作结果：由稀疏矩阵copy构造新稀疏矩阵——复制构造函数
{
	...
	for (int i = 0; i < num; i++) {
		...
		cout << triElems[i].row << " " << triElems[i].col << " " << triElems[i].value << endl;
	}	// 复制三元组
}
...
// 操作结果：将稀疏矩阵copy赋值给当前稀疏矩阵——赋值运算符重载
{
	if (&copy != this) {
		maxSize = copy.maxSize;						// 最大非零元素个数
		if (triElems != NULL) delete[]triElems;	// 释放存储空间
		triElems = new Triple<ElemType>[30];	// 分配存储空间
		rows = copy.rows;							// 复制行数
		cols = copy.cols;							// 复制列数
		num = copy.num;								// 复制非零元素个数
...
TriSparseMatrix<ElemType> TriSparseMatrix<ElemType>::operator +(const TriSparseMatrix<ElemType>& copy) {
	    int k = num;
		TriSparseMatrix<ElemType> x;
		x.triElems = new Triple<ElemType>[maxSize + copy.maxSize];
		x.cols = cols;
		x.rows = rows;
		x.num = num + copy.num;
		x.maxSize = maxSize + copy.maxSize;
		for (int i = 0; i < num; i++) {
			x.triElems[i] = triElems[i];
		}
		for (int j = 0; j < copy.num; j++) {
			for (int i = 0; i < num; i++) {
				if (copy.triElems[j].col == triElems[i].col && copy.triElems[j].row == triElems[i].row) {
					x.triElems[i].value += copy.triElems[j].value;
					break;
				}
				else if (i == num - 1) {
					x.triElems[k++] = copy.triElems[j];
				}
			}
		}
		x.num = k;
		for (int i = 0; i < x.num-1; i++) {
			for (int j = i+1; j < x.num; j++) {
				if (x.triElems[i].row > x.triElems[j].row) {
					Swap(x.triElems[i], x.triElems[j]);
				}
			}
		}
		return x;
